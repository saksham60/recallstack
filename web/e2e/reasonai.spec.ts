...
test("analysis panel moves, resizes, remembers expanded size and stays inside the editor", async ({ authenticatedPage: page }) => {
  test.setTimeout(60_000);
  await page.setViewportSize({ width: 1600, height: 1000 });
  const document = analysisDocument();
  const key = LocalStorageSystemDesignRepository.storageKey("url-shortener");
  ...
  await page.getByLabel("Message ReasonAI").fill("Show failure impact");
  await page.getByLabel("Message ReasonAI").press("Enter");
  const panel = page.getByRole("region", { name: "ReasonAI analysis", exact: true });
  await expect(panel).toBeVisible();
  await page.getByRole("button", { name: "Close ReasonAI" }).click();
  await panel.getByRole("button", { name: "Expand analysis details" }).click();
  const before = (await panel.boundingBox())!;
  const handle = panel.getByRole("button", { name: "Move analysis panel" });
  const grip = (await handle.boundingBox())!;
  await page.mouse.move(grip.x + 50, grip.y + 15);
  await page.mouse.down();
  await page.mouse.move(grip.x + 240, grip.y + 85, { steps: 8 });
  await page.mouse.up();
  const moved = (await panel.boundingBox())!;
  expect(moved.x).toBeCloseTo(before.x + 190, 0);
  expect(moved.y).toBeCloseTo(before.y + 70, 0);
  const resize = panel.getByRole("button", { name: "Resize analysis panel" });
  const corner = (await resize.boundingBox())!;
  await page.mouse.move(corner.x + 10, corner.y + 10);
  await page.mouse.down();
  await page.mouse.move(corner.x + 150, corner.y + 90, { steps: 8 });
  await page.mouse.up();
  const resized = (await panel.boundingBox())!;
  expect(resized.width).toBeCloseTo(moved.width + 140, 0);
  expect(resized.height).toBeCloseTo(moved.height + 80, 0);
  await panel.getByRole("button", { name: "Collapse analysis details" }).click();
  expect((await panel.boundingBox())!.height).toBeLessThan(140);
  await panel.getByRole("button", { name: "Expand analysis details" }).click();
  ...
  expect((await panel.boundingBox())!.width).toBeCloseTo(resized.width + 16, 0);
  await handle.focus(); await page.keyboard.press("ArrowRight");
  expect((await panel.boundingBox())!.x).toBeCloseTo(resized.x + 16, 0);
  ...
  await expect.poll(() => panel.evaluate((element) => {
    ...
    return rect.left >= parent.left && rect.top >= parent.top && rect.right <= parent.right + 1 && rect.bottom <= parent.bottom + 1;
  })).toBe(true);
  await expect(page.getByRole("button", { name: "Undo", exact: true })).toBeDisabled();
