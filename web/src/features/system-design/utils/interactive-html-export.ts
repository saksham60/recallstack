...
(function(){
  ...
  function dash(edge,resolved){if(Array.isArray(edge.dashPattern)&&edge.dashPattern.length)return edge.dashPattern.join(' ');var unit=Math.max(1,resolved.strokeWidth),patterns={dashed:(unit*4)+' '+(unit*3),dotted:unit+' '+(unit*2.5),dash_dot:(unit*5)+' '+(unit*2.5)+' '+unit+' '+(unit*2.5)},pattern=patterns[resolved.lineStyle]||'';if(!pattern&&resolved.animationMode==='moving_dots')return unit+' '+(unit*3);if(!pattern&&(resolved.animationMode==='moving_dash'||resolved.animationMode==='direction_pulse'))return(unit*5)+' '+(unit*3);return pattern;}
  ...
  function drawEdge(edge,nodeMap){var source=nodeMap[edge.sourceNodeId],target=nodeMap[edge.targetNodeId];if(!source||!target||source.visible===false||target.visible===false)return;var resolved=resolvedEdge(edge),color=edgeColor(edge),animation=resolved.animationMode!=='none',group=el('g',{'class':'edge','data-id':edge.id,'data-render-plane':'edges','data-routing':resolved.routing,'data-line-style':resolved.lineStyle,'data-animation':resolved.animationMode,'data-label-position':resolved.labelPosition,'data-read-only':'true'},world),path=el('path',{d:edgePath(edge,source,target,resolved.routing),fill:'none',stroke:color,'stroke-width':resolved.strokeWidth,'stroke-opacity':resolved.opacity,'stroke-dasharray':dash(edge,resolved),'stroke-linecap':'round','stroke-linejoin':'round','vector-effect':'non-scaling-stroke','class':'edge-path'+(animation?' edge-animation edge-animation-'+resolved.animationMode:''),'marker-start':marker(resolved.startArrowhead,'start',color),'marker-end':marker(resolved.endArrowhead,'end',color)},group);if(animation){path.style.animationDuration=(1.4/resolved.animationSpeed)+'s';path.style.animationDirection=resolved.animationDirection==='forward'?'normal':resolved.animationDirection;}
  ...
  document.body.classList.toggle('document-hidden',document.hidden);renderActiveDiagram();requestAnimationFrame(fit);
})();`;
