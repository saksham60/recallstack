...
(function(){
  ...
  function surface(group,node,visual){var w=node.width,h=node.height,style=node.style||{},accent=visual.accent||'#a78bfa',soft=visual.softAccent||'#2e2350',fill=style.fill||'#18181b',stroke=style.stroke||accent,sw=number(style.strokeWidth,1),radius=number(style.borderRadius,10),chrome=visual.chrome,dashes=borderDash(node,''),common={fill:fill,stroke:stroke,'stroke-width':sw,'stroke-dasharray':dashes};
    if(node.type==='freehand'&&node.drawing){var ink=node.drawing,coordinates=[];for(var pointIndex=0;pointIndex<ink.points.length;pointIndex+=2){coordinates.push(ink.points[pointIndex]+','+ink.points[pointIndex+1]);}var pointList=coordinates.join(' '),inkOpacity=ink.opacity===undefined?1:ink.opacity,inkStyle=ink.lineStyle||'solid',inkPatterns={dashed:(ink.strokeWidth*4)+' '+(ink.strokeWidth*3),dotted:ink.strokeWidth+' '+(ink.strokeWidth*2.5),dash_dot:(ink.strokeWidth*5)+' '+(ink.strokeWidth*2.5)+' '+ink.strokeWidth+' '+(ink.strokeWidth*2.5)},inkDash=Array.isArray(ink.dashPattern)&&ink.dashPattern.length?ink.dashPattern.join(' '):(inkPatterns[inkStyle]||''),inkMode=ink.animationMode||'moving_dash',guide=el('polyline',{points:pointList,fill:'none',stroke:ink.stroke,'stroke-width':ink.strokeWidth,'stroke-opacity':inkOpacity,'stroke-dasharray':inkDash,'stroke-linecap':'round','stroke-linejoin':'round','data-freehand':'true'},group);if(inkMode!=='none'){var motionDash=inkMode==='moving_dots'?'1 11':(inkMode==='moving_dash'?(inkDash||((ink.strokeWidth*5)+' '+(ink.strokeWidth*3))):''),motion=el('polyline',{points:pointList,fill:'none',stroke:ink.stroke,'stroke-width':ink.strokeWidth+1,'stroke-opacity':inkMode==='flow_pulse'?.3*inkOpacity:.85*inkOpacity,'stroke-dasharray':motionDash,'stroke-linecap':'round','stroke-linejoin':'round','class':'edge-motion freehand-motion','data-motion-kind':inkMode},group);animatedEdges.push({mode:inkMode,motion:motion,guide:guide,speed:ink.animationSpeed||1,direction:ink.animationDirection||'forward',strokeWidth:ink.strokeWidth});}return;}
