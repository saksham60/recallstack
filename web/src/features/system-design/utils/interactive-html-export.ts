...
(function(){
  ...
  function surface(group,node,visual){var w=node.width,h=node.height,style=node.style||{},accent=visual.accent||'#a78bfa',soft=visual.softAccent||'#2e2350',fill=style.fill||'#18181b',stroke=style.stroke||accent,sw=number(style.strokeWidth,1),radius=number(style.borderRadius,10),chrome=visual.chrome,dashes=borderDash(node,''),common={fill:fill,stroke:stroke,'stroke-width':sw,'stroke-dasharray':dashes};
    if(node.type==='freehand'&&node.drawing){var ink=node.drawing,coordinates=[];for(var pointIndex=0;pointIndex<ink.points.length;pointIndex+=2){coordinates.push(ink.points[pointIndex]+','+ink.points[pointIndex+1]);}var pointList=coordinates.join(' '),inkOpacity=ink.opacity===undefined?1:ink.opacity,guide=el('polyline',{points:pointList,fill:'none',stroke:ink.stroke,'stroke-width':ink.strokeWidth,'stroke-opacity':inkOpacity,'stroke-linecap':'round','stroke-linejoin':'round','data-freehand':'true'},group),motion=el('polyline',{points:pointList,fill:'none',stroke:ink.stroke,'stroke-width':ink.strokeWidth+1,'stroke-opacity':.85*inkOpacity,'stroke-dasharray':(ink.strokeWidth*5)+' '+(ink.strokeWidth*3),'stroke-linecap':'round','stroke-linejoin':'round','class':'edge-motion freehand-motion','data-motion-kind':'moving_dash'},group);animatedEdges.push({mode:'moving_dash',motion:motion,guide:guide,speed:1,direction:'forward',strokeWidth:ink.strokeWidth});return;}
