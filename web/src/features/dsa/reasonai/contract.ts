export const DSA_ACTIONS = ["chat", "hint", "explain", "start", "trace", "review", "complexity", "research", "solution"] as const;
...
export interface DSATutorSource { title: string; url: string }
export interface DSATutorResponse {
  text: string;
  sources: DSATutorSource[];
  webStatus: "off" | "used" | "unavailable" | "empty";
  notice?: string;
}
...
export function parseDSATutorRequest(value: unknown): DSATutorRequest {
  ...
  only(input, ["action", "message", "searchWeb", "hintLevel", "context", "history"]);
