...
class SyncEngine {
  ...
  Future<SyncResult> runSync() async {
    ...
    try {
      ...
      final result = type == SyncErrorType.serverFailure
  ...
  Future<void> _pushMutations() async {
    ...
    for (final m in pendingMutations) {
      ...
      try {
        ...
        if (decoded is! Map<String, dynamic>) {
          invalidMutationReasons[m.mutationId] =
              'invalid_mutation_payload_shape';
          continue;
        }
    ...
    if (invalidMutationReasons.isNotEmpty) {
      await _db.transaction(() async {
        for (final entry in invalidMutationReasons.entries) {
          await (_db.update(
            _db.localOutbox,
          )..where((t) => t.mutationId.equals(entry.key))).write(
    ...
    try {
      ...
      final rawResults = responseData is Map<String, dynamic>
          ? responseData['results']
          : null;
      final results = rawResults is List<dynamic>
          ? rawResults
          : const <dynamic>[];
      ...
      await _db.transaction(() async {
        for (final result in results) {
          ...
          if (status is! String) {
            continue; // If status missing/wrong type, skip (becomes missing)
          }

          validReturnedMutationIds.add(mId); // Now it's safely valid

          final rawErrorMessage = result['error_code'];
          final errorMessage = rawErrorMessage is String
              ? rawErrorMessage
              : null;
  ...
  Future<void> _pullCatalogChanges(String domainId) async {
    ...
    while (hasMoreChanges) {
      ...
      if (nextCursor == null || nextCursor.isEmpty) {
        throw const FormatException('missing catalog next_cursor');
      }
      hasMoreChanges = data['has_more'] == true;
      if (hasMoreChanges && nextCursor == cursor) {
        throw const FormatException('catalog cursor did not advance');
      }
      cursor = nextCursor;
    }
  ...
  Future<void> _pullUserChanges() async {
    ...
    while (hasMoreChanges) {
      ...
      if (data['full_resync_required'] == true) {
        throw StateError(
          'USER FULL RESYNC BLOCKED BY EXISTING BACKEND CONTRACT',
        );
      }
      final changes = data['changes'] as List<dynamic>? ?? [];
      ...
      final nextCursor = data['next_cursor']?.toString();
      if (nextCursor == null || nextCursor.isEmpty) {
        throw const FormatException('missing user next_cursor');
      }
      hasMoreChanges = data['has_more'] == true;
      if (hasMoreChanges && nextCursor == cursor) {
        throw const FormatException('user cursor did not advance');
      }
      cursor = nextCursor;
    }
  ...
  Future<void> _handleFullResync(String domainId) async {
    ...
    // Probe the existing cursor contract after the complete snapshot has been
    // downloaded. Network, auth, and server errors propagate with their normal
    // SyncResult semantics while the old database remains untouched.
    final cursorResponse = await _apiClient.client.get(
      '/sync/catalog/$domainId',
      queryParameters: {'device_id': deviceState.deviceId},
    );
    final cursorData = cursorResponse.data as Map<String, dynamic>;

    // The current backend keeps full_resync_required set for this device and
    // exposes no snapshot-bound cursor or acknowledgement endpoint. Neither a
    // paged response nor a terminal feed cursor obtained from an omitted
    // `after` value can be proven to describe the newer snapshot downloaded
    // above. Preserve the existing catalog and cursor until the backend can
    // provide that proof.
    if (cursorData['full_resync_required'] == true) {
      throw StateError(
        'full_resync_required remained set after snapshot download',
      );
    }
    if (cursorData['has_more'] == true) {
      throw StateError(
        'paged catalog feed is not a safe terminal snapshot cursor',
      );
    }
    throw StateError(
      'FULL RESYNC TERMINAL CURSOR BLOCKED BY EXISTING BACKEND CONTRACT',
    );
  }
}
