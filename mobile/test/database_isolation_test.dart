  ...
  group('Database Isolation', () {
    test('Authenticated users have isolated sqlite files', () async {
      ...
      Future<void> insertFkData(AppDatabase db) async {
        await db.into(db.categories).insert(CategoriesCompanion.insert(id: 'cat-1', domainId: 'domain-1', title: 'Category', sortOrder: 0, updatedAt: DateTime.now()));
        await db.into(db.contentItems).insert(ContentItemsCompanion.insert(id: 'item-1', categoryId: 'cat-1', title: 'Item', slug: 'item', type: 'concept', sortOrder: 0, updatedAt: DateTime.now()));
      }
      ...
      await dbA.into(dbA.bookmarks).insert(BookmarksCompanion.insert(
        contentId: 'item-1',
        createdAt: DateTime.now(),
      ));

      await dbA.into(dbA.userProgress).insert(UserProgressCompanion.insert(
        contentId: 'item-1',
        status: 'learning',
        rowVersion: 1,
        updatedAt: DateTime.now(),
      ...
      await dbA.into(dbA.reviewCards).insert(ReviewCardsCompanion.insert(
        id: 'card-A',
        contentId: 'item-1',
        state: 'due',
        rowVersion: 1,
      ...
      await dbA.into(dbA.localOutbox).insert(LocalOutboxCompanion.insert(
        mutationId: 'mut-A',
      ...
      await dbB.into(dbB.bookmarks).insert(BookmarksCompanion.insert(
        contentId: 'item-1',
        createdAt: DateTime.now(),
      ));
      await dbB.into(dbB.userProgress).insert(UserProgressCompanion.insert(
        contentId: 'item-1',
        status: 'mastered',
        rowVersion: 1,
        updatedAt: DateTime.now(),
      ));
      await dbB.into(dbB.reviewCards).insert(ReviewCardsCompanion.insert(
        id: 'card-B',
        contentId: 'item-1',
        state: 'pending_sync',
        rowVersion: 1,
      ));
      await dbB.into(dbB.localOutbox).insert(LocalOutboxCompanion.insert(
        mutationId: 'mut-B',
