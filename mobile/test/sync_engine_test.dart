...
void main() {
  ...
  test(
    ...
    () async {
      ...
      expect(
        result,
        equals(SyncResult.serverFailure),
  ...
  test('malformed JSON payload is rejected without failing batch', () async {
    ...
    when(
      ...
      expect(
        (mutations.first as Map<String, dynamic>)['mutation_id'],
        'valid-1',
      );
      return Response(
        requestOptions: RequestOptions(path: ''),
        data: {
          'results': [
            {'mutation_id': 'valid-1', 'status': 'applied'},
          ],
        },
      );
    });
    ...
    final invalid = await (db.select(
      db.localOutbox,
    )..where((t) => t.mutationId.equals('invalid-2'))).getSingle();
    ...
    final valid = await (db.select(
      db.localOutbox,
    )..where((t) => t.mutationId.equals('valid-1'))).getSingleOrNull();
    expect(valid, isNull);
  });

  test(
    'batch response parsing exact semantics (unknown ID, missing status, duplicate ID)',
    () async {
      const user = supabase.User(
        id: 'test_user',
        appMetadata: {},
        ...
        return Response(
          ...
          data: {
            'results': [
              ...
              {
                'mutation_id': 'unknown-3',
                'status': 'applied',
              }, // Unknown, should be ignored
              {
                'mutation_id': 'm2',
              }, // Missing status, should be ignored (making m2 retryable)
            ],
          },
        );
      });
      ...
      final m1 = await (db.select(
        db.localOutbox,
      )..where((t) => t.mutationId.equals('m1'))).getSingleOrNull();
      expect(m1, isNull);

      // m2 missing status in response, becomes retryable
      final m2 = await (db.select(
        db.localOutbox,
      )..where((t) => t.mutationId.equals('m2'))).getSingle();
  ...
  test(
    'full resync aborts if full_resync_required=true on cursor fetch',
    () async {
      const user = supabase.User(
        id: 'test_user',
        appMetadata: {},
      ...
      when(
        ...
        return Response(
          requestOptions: RequestOptions(path: ''),
          data: {'full_resync_required': true},
        );
      });

      // We fetch categories and content
      when(() => mockDio.get('/domains/dsa/categories')).thenAnswer(
        (_) async => Response(
          requestOptions: RequestOptions(path: ''),
          data: [],
        ),
      ...
      when(
        () => mockDio.get(
          '/sync/catalog/dsa',
          queryParameters: {'device_id': 'dev-1'},
        ),
      ).thenAnswer(
        (_) async => Response(
          requestOptions: RequestOptions(path: ''),
